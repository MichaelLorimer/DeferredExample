...
void CMesh ::SetPos(CVector3 position)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.SetPosition(CVector3(position));
	}
}

void CMesh::SetRot(CVector3 rotation)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.RotateLocalX(rotation.x);
		GetNode(i).positionMatrix.RotateLocalY(rotation.y);
		GetNode(i).positionMatrix.RotateLocalZ(rotation.z);
		
	}
}

void CMesh::SetScale(float scale)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.SetScale(scale);
	}
}

void CMesh::SetScaleX(float scale)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.SetScaleX(scale);
	}
}

void CMesh::SetScaleY(float scale)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.SetScaleY(scale);
	}
}

void CMesh::SetScaleZ(float scale)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.SetScaleZ(scale);
	}
}

void CMesh:: SetFacing(CVector3 target)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.FaceTarget(target);
	}
}

void CMesh:: SetDirection(CVector3 target)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.FaceDirection(target);
	}
}

void CMesh::MoveLocalXYZ (float x, float y, float z)
{
	int numNode = GetNumNodes();
	for (int i = 0; i < numNode; i++)
	{
		GetNode(i).positionMatrix.MoveLocalX(x);
		GetNode(i).positionMatrix.MoveLocalY(y);
		GetNode(i).positionMatrix.MoveLocalZ(z);
	}
}